...
(function() {
  ...
  _.invoke = function(collection, functionOrKey, args) {
    return _.map(collection, function(el){
      return (typeof functionOrKey === 'function' ? functionOrKey : el[functionOrKey]).apply(el, args);
    });
  };
  ...
  _.reduce = function(collection, iterator, accumulator) {
    var intitial = arguments.length > 2;
    _.each(collection, function(el){
      if (!intitial){
        accumulator = el;
        intitial = true;
  ...
  _.every = function(collection, iterator) {
    ...
    if (Object.keys(collection).keys === 0){return true;}
  ...
  _.shuffle = function(array) {
    ...
    return _.reduce(array, function(a, b){
      var item = Math.floor(Math.random() * temp.length);
      a.push(temp[item]);
      temp.splice(item, 1);
      return a;
    }, []);
  ...
  _.zip = function() {
    var len = _.reduce(arguments, function(a, b){
    ...
    for (var i = 0; i < result.length; i++){
      ...
      for (var j = 0; j < arguments.length; j++){
        result[i].push(arguments[j][i] || undefined);
      }
    }
    return result;
  };
  ...
  _.intersection = function() {
    var check = Array.prototype.slice.call(arguments);
    var result = [];
    //build object
    var list = _.reduce(_.flatten(check), function(a, b){
      if (a[b] === undefined){
        a[b] = 1;
      }else{
        a[b]++;
    ...
    //parse object
    _.each(list, function(el, pos){
      if (list[pos] === check.length){
        result.push(pos);
      }
    });
    return result;
